...
def plot():
    """
    Plot concentration inequalities for various bounds on a Chi-square
    random variable with n degrees of freedom
    :return:
    """
    # produce separate plots for each n
    for n in [1, 2, 4, 8]:
        plt.figure()
        ...
        # plot for x from just after 0 to 4n (to avoid possible div by 0)
        x_stop = 4 * n
        x_range = np.linspace(0.0001, x_stop, 100)

        # simulated/empirical prob of chi-square with n dof
        ...
        # Chernoff bound: e^(-n x^2 /(2n + 2x) (not correct as it falls bellow empirical bound)
        ...
        plt.ylim(0.0001, 20)  # Set maximum y-value
        plt.grid(True)
        plt.tight_layout()
